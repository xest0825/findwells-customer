...
<html>
	<head>
				...
				success : function(data){
					...
					if (data.results != null && data.results != undefined && data.results.length != 0){
						for (var i = 0; i<data.results.length; i++) {
							...
							if (board.FOLDER_YN == 'N' && board.ISLEAF == 1 && board.LV == 2) {
		...
		function getItemList(){
			...
			var params = { BD_NO : "62", SEARCH_WORD: $('#search_word').val(), page: page, pageSize: pageSizeTmp}
			$.ajax({
				...
				success : function(data){
					...
					for (var i = 0; i<item.length; i++) {
						html += ''					
							 +	'<li>'
							 +	'	<a href="/mobile/report_detail.go?BD_NO='+item[i].BD_NO +'&ITEM_NO=' + item[i].ITEM_NO+ '&page=' + pageTmp + '">'
