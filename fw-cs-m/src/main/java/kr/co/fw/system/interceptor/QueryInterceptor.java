package kr.co.fw.system.interceptor;
...
public class QueryInterceptor implements Interceptor {
	...
	public Object intercept(Invocation invocation) throws Throwable {
		if (kr.co.fw.config.Constants.QueryLogging == LOGGING.NOLOGGING) { // 쿼리로그 비활성화시 return
		...
		// String paramStr = handler.getParameterHandler().getParameterObject() != null
		...
		try {
				...
				if (param instanceof Integer || param instanceof Long || param instanceof Float
					...
					for (ParameterMapping mapping : paramMapping) {
						...
						try {
							...
							field = paramClass.getSuperclass().getDeclaredField(propValue); // MB_ID 같이 부모(BaseVO)에 있는
																							// 변수들을 얻어옴
						}

						field.setAccessible(true); // 멤버변수의 접근자가 private일 경우 reflection을 이용하여 값을 해당 멤버변수의 값을 가져오기 위해 별도로
													// 셋팅
			...
			log.info("====================================");
			log.info("Method=[{}], ID=[{}]", invocation.getMethod().getName(), ms.getId());
			log.info("Query\n{}", sql);
			if (param != null)
