...
public class CommUtil {
	 ...
	 * @return 결과 calendar객체
	 */
	public static Calendar getCalendarInstance(int year, int month, int date, int hour, int minute, int second) {
		Calendar retCal = Calendar.getInstance();
		month--;
	...
	public String formatDate(String date, String separator) {
		...
		sb.append(date.substring(0, 4)).append(separator).append(date.substring(4, 6)).append(separator)
     ...
     * genexon.alert 호출
     */
    public static void sendGenexonAlert(HttpServletResponse response, String info, String contents, String message) throws Exception {
		...
		try {
			...
			sContents += "<script type='text/javascript' src='/js/genexon.js'></script>";
			sContents += "<script type='text/javascript'>";
	...
	public boolean callShellCommand(String ContentsDwonloadPath, String url) throws Exception {
		...
		boolean result = true;
		String command = ContentsDwonloadPath + "getContents.sh " + ContentsDwonloadPath + "temp.html " + url + " "
				+ ContentsDwonloadPath + "temputf8.html";
	 ...
	 * 개발: sja
	 * 
	 * @date 2014. 4. 22. : 오후 4:09:10 file : CommUtil.java
	 * @param map
	 * @param objClass
	 * @return
	 */
...
//                        }
	...
	/**
	 * SONObject를 Map<String, String> 형식으로 변환처리.
	 * 
	 * @param JSONObject
...
//					jsonObj = (JSONObject) parser.parse(param);
	...
	/**
	 * HttpURLConnection으로 okname모듈 데이터 받아오기(JSONObject 형식)
	 * 
	 * @param Map
	 * @return JSONObject
	 */
	public static JSONObject getApiOknameData(String apiUrl, String reqMethod, Map<String, String> paramMap)
			throws IOException, Exception {
		JSONObject jsonObj = new JSONObject();
		BufferedReader bReader = null;
		OutputStream os = null;
	...
	public static String maskingPhoneNumber(String phoneNumber) {
		...
		// 공백제거
		maskedPhoneNumber = maskedPhoneNumber.replaceAll(" ", "");

		// '-'가 포함되어있으면 모두 삭제
		if (maskedPhoneNumber.contains("-")) {
			maskedPhoneNumber = maskedPhoneNumber.replaceAll("[^0-9]", "");
		}

		// 11자리가 되지않으면 그냥 반환
		if (maskedPhoneNumber.length() != 11) {
			maskedPhoneNumber = phoneNumber;
		} else {
			String num1 = maskedPhoneNumber.substring(0, 3);
			String num2 = maskedPhoneNumber.substring(3, 7);

			maskedPhoneNumber = num1 + "-" + num2 + "-****";
		}
